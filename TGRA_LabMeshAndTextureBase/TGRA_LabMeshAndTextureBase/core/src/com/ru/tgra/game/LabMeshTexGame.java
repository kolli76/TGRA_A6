...
public class LabMeshTexGame extends ApplicationAdapter implements InputProcessor {
	...
	/*	
		...
		tent = new Tentacle(3, 1.0f, new Point3D(5,5,5));
		targetAngle = 0;
	...
	private void update()
	{
		...
		targetAngle += deltaTime*0.1f;
		
		target.set((float)Math.cos(targetAngle), (float)Math.sin(targetAngle), (float)Math.sin(targetAngle));
		tent.follow(target);
